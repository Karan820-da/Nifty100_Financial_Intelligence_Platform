...
df = df.sort_values(["company_id", "year_dt"])
...
for company, group in df.groupby("company_id"):
    ...
    for i in range(5, len(group)):
        ...
        revenue_cagr = calculate_cagr(previous["sales"], current["sales"], 5)

        pat_cagr = calculate_cagr(previous["net_profit"], current["net_profit"], 5)

        eps_cagr = calculate_cagr(previous["eps"], current["eps"], 5)

        updates.append([revenue_cagr, pat_cagr, eps_cagr, company, current["year"]])

update_df = pd.DataFrame(
    updates,
    columns=["revenue_cagr_5yr", "pat_cagr_5yr", "eps_cagr_5yr", "company_id", "year"],
)
...
print(update_df[update_df.isnull().any(axis=1)])
...
cursor.executemany(sql, update_df.values.tolist())
