...
profit = pd.read_sql("SELECT company_id,year,net_profit FROM profitandloss", engine)
...
df = cashflow.merge(profit, on=["company_id", "year"], how="left")

df = df.merge(companies, on="company_id", how="left")

df["company_name"] = df["company_name"].fillna(df["company_id"]).str.strip()
...
for company in sorted(df["company_id"].unique()):
    ...
    company_df = df[df["company_id"] == company].sort_values("year_num")
...
patterns_df.to_csv(output_dir / "capital_allocation.csv", index=False)

latest_year = patterns_df["year_num"].max()

latest = patterns_df[patterns_df["year_num"] == latest_year]

distribution = latest["capital_allocation"].value_counts().reset_index()

distribution.columns = ["Capital Allocation Pattern", "Companies"]

print("\nLatest Year Distribution\n")

print(distribution)

distribution.to_csv(output_dir / "capital_allocation_distribution.csv", index=False)

changes = []

for company in patterns_df["company_id"].unique():

    company_df = patterns_df[patterns_df["company_id"] == company].sort_values(
        "year_num"
    )
        ...
        changes.append(
            {
                ...
                "previous_pattern": previous["capital_allocation"],
                "latest_pattern": latest["capital_allocation"],
            }
        )
