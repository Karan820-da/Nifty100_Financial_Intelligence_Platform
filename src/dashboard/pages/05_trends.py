...
st.write("Analyze historical trends across the Nifty100 companies.")
...
metric = st.selectbox(
    "Select Metric",
    ["return_on_equity_pct", "pe_ratio", "market_cap_crore", "composite_quality_score"],
)
trend_df = df.groupby("year")[metric].mean().reset_index()

fig = px.line(
    trend_df, x="year", y=metric, markers=True, title=f"Average {metric} Over Time"
)
...
fig = px.bar(top10, x="company_id", y=metric, color="broad_sector")
